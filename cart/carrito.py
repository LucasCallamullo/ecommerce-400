...
class Carrito:
    ...
    def total_price(self) -> float:
        ...
        """
        t = sum(item['price'] * item['qty'] for item in self.carrito.values()) if self.carrito else 0
        return float(t)
    ...
    def total_items(self) -> int:
        ...
        """
        t = sum(item['qty'] for item in self.carrito.values()) if self.carrito else 0
        return t
    ...
    def save_item(self, product=None):
        ...
        if not (self.user.is_authenticated and self.cart_id):
            return
    ...
    def add_product(self, product, quantity=1) -> bool:
        ...
        if product_id not in self.carrito:
            ...
            self.carrito[product_id]["qty"] += quantity
    ...
    def subtract_product(self, product, qty=1) -> bool:
        ...
        if self.carrito[product_id]["qty"] > 1:
